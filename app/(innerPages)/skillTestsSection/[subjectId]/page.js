...
function Page({ params }) {
    ...
    const getQuizData = async () => {
      ...
      try {
        const token =
          ...
          console.log("test Id froionmsection", subjectId);
          
        const resp = await GlobalApi.GetSkillTestsData(subjectId, token);
  ...
  const quizProgressSubmit = async (data) => {
    ...
    try {
      const token =
        typeof window !== "undefined" ? localStorage.getItem("token") : null;
      const resp = await GlobalApi.SkillTestProgress(data, token);
  ...
  const quizSubmit = async () => {
    ...
    const token =
      typeof window !== "undefined" ? localStorage.getItem("token") : null;
    try {
      const resp = await GlobalApi.UpdateSkillTestData(token, subjectId);
      if (resp && resp.status === 201) {

...
function RoadMap({ selectedCareer }) {
  ...
  const handleComplete = (tab, milestoneId, description, careerName) => {
    ...
    setPendingMilestone({ tab, milestoneId, description, careerName });
  ...
  const handleConfirm = () => {
    if (pendingMilestone) {
      saveMilestone(
        pendingMilestone.tab,
        pendingMilestone.milestoneId,
        pendingMilestone.description,
        pendingMilestone.careerName,
        { global: false, countrySpecific: false }
      );
    }
    ...
    <div className="w-full mx-auto">
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          />
          <div className="relative bg-gray-800 border border-gray-700/50 rounded-2xl p-6 w-full max-w-md shadow-2xl">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-full bg-orange-500/20 flex items-center justify-center flex-shrink-0">
                <CheckCircle className="w-5 h-5 text-orange-400" />
              </div>
              <h2 className="text-white text-lg font-semibold">Complete Milestone</h2>
            </div>

            <p className="text-gray-400 text-sm mb-2">Are you sure you want to mark this as complete?</p>
            <p className="text-white text-sm font-medium bg-gray-700/50 rounded-xl px-4 py-3 mb-6 leading-relaxed">
              "{pendingMilestone?.description}"
            </p>
            ...
            <div className="flex gap-3">
              <button
                onClick={handleCancel}
                className="flex-1 py-2.5 px-4 rounded-xl border border-gray-600/50 text-gray-300 hover:text-white hover:bg-gray-700/50 transition-all duration-200 text-sm font-medium"
              >
                Cancel
              </button>
              <button
                onClick={handleConfirm}
                className="flex-1 py-2.5 px-4 rounded-xl bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white font-semibold text-sm transition-all duration-200 shadow-lg shadow-orange-500/25"
              >
                Confirm
              </button>
            </div>
      ...
      {/* Community modal temporarily disabled — will revisit when community concept is updated */}
      {/* {showCommunityModal && (
        <SelectCommunity
          handleComplete={() => {

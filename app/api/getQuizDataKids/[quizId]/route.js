...
import { ANALYTICS_QUESTION_KIDS, ANSWERS, OPTIONS_KIDS, QUESTIONS, QUIZ_SEQUENCES, TASKS, USER_DETAILS, USER_PROGRESS, USER_TASKS } from '@/utils/schema';
...
export async function GET(req, { params }) {
    ...
    try {
        ...
        const quizWithOption = await db
            .select({
                questionId: ANALYTICS_QUESTION_KIDS.id,
                questionText: ANALYTICS_QUESTION_KIDS.question_text,
                answerId: OPTIONS_KIDS.id,
                answerText: OPTIONS_KIDS.option_text,
                analyticId: OPTIONS_KIDS.analytic_id

            })
            .from(ANALYTICS_QUESTION_KIDS)
            .leftJoin(OPTIONS_KIDS, eq(ANALYTICS_QUESTION_KIDS.id, OPTIONS_KIDS.question_id))
            .where(eq(ANALYTICS_QUESTION_KIDS.quiz_id, quizId))
            .execute();

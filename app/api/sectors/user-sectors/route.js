...
export const dynamic = "force-dynamic";
  ...
  export async function POST(req) {
    ...
    try {
      ...
      const { plan_type, country, class_id } = userDetails[0];
        ...
        const existingNewCommunity = await db
          ...
          .where(
            and(
              ...
              eq(COMMUNITIES.type, "sector"),
              eq(COMMUNITIES.ref_id, sectorToAdd.sector_id)
            )
          )
          .execute();
        ...
        if (existingNewCommunity.length > 0) {
          ...
          const [newCommunityInsert] = await db
            ...
            .values({
              class_id: class_id,
              type: "sector",
              ref_id: sectorToAdd.sector_id,
              name: addedSector[0].name,
              description: `Community for ${addedSector[0].name} sector`,
            })
            .execute();

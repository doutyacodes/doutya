...
export async function fetchAndSaveRoadmap(userId, scopeId, birth_date, age, parentScopeId, scopeName, type1, type2, language, scopeType = "career") {
    console.log(`scopeId: ${scopeId}, age: ${age}, scopeType: ${scopeType}, scopeName: ${scopeName}, type1: ${type1}, type2: ${type2}`);
    
    try {
        const currentAgeWeek = getCurrentWeekOfAge(birth_date);
        
        // Use the modified generateRoadmapPrompt function that accepts scopeType and scopeName
        const prompt = await generateRoadmapPrompt(
            userId, scopeType, scopeName, type1, type2, age, currentAgeWeek, language, languageOptions
        );
                                ...
                                for (const [subcategory, milestones] of Object.entries(subcategories)) {
                                    ...
                                    if (subcategory !== "Certification Milestones") {
                                        ...
                                        for (const certification of milestones) {
                                            ...
                                            // Check if a certification with the same name, age, and scope already exists
                                            const existingCertification = await db
                                                .select()
                                                .from(CERTIFICATIONS)
                                                .where(
                                            ...
                                            if (milestoneAge === age) {
                                                milestonesForFrontend.push({
                                                    ...
                                                    milestoneCompletionStatus: false,
                                                    milestoneDateAchieved: null,
                                                    // certificationId: certificationId, // Include certification ID for frontend

...
const fetchSubjectsFromOpenAI = async (
  userId,
  scopeName,
  country,
  birthDate,
...
const saveSubjectsToDatabase = async (
  ...
  try {
    ...
    console.log(
      `${scopeType} subjects and their relationships saved successfully.`
    );
  } catch (error) {
...
export const processCareerSubjects = async (
  ...
  try {
    ...
    console.log(
      `Subject generation completed successfully for keyHash: ${keyHash}`
    );
  } catch (error) {
    console.error(
      `Error processing ${scopeType} subjects for keyHash: ${keyHash}:`,
      error
    );
    ...
    if (keyHash) {
      try {
        ...
        console.log(
          `Updated generation status to failed for keyHash: ${keyHash}`
        );
      } catch (updateError) {
        console.error(
          `Failed to update generation status for keyHash: ${keyHash}:`,
          updateError
        );
      }
    }
